...
HRESULT CWaveFile::SetBeginSamples(DWORD dwBeginSamples)
{
	DWORD dwBeginBytes = dwBeginSamples*m_pwfx->nBlockAlign;

    if( m_bIsReadingFromMemory )
    {
		if( dwBeginBytes >= m_ulDataSize)
        {
            m_pbDataCur = m_pbData + m_ulDataSize;
        }
		else
		{
			m_pbDataCur = m_pbData + dwBeginBytes;
		}        
    }
    else 
    {
        ...
        if( m_dwFlags == WAVEFILE_READ )
        {
            ...
            if( -1 == mmioSeek( m_hmmio, m_ck.dwDataOffset + dwBeginBytes,
				SEEK_SET ) )
                return DXTRACE_ERR( TEXT("mmioSeek"), E_FAIL );
			m_ck.cksize -= dwBeginBytes;
        }
    }
